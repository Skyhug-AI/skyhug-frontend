...
const TherapistCard: React.FC<TherapistCardProps> = ({
  ...
  return <Card className="overflow-hidden flex flex-col items-center p-6 h-[280px] w-[220px] rounded-2xl bg-white hover:scale-[1.03] hover:shadow-lg transition-all duration-300 cursor-pointer relative group" onClick={onClick}>
      ...
      <main className="flex-grow flex flex-col items-center justify-center px-4 py-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl justify-items-center">
          ...
          animationDelay: `${index * 0.1}s`
              ...
              <TherapistCard id={therapist.id} name={therapist.name} description={therapist.description} specialties={therapist.specialties} avatarSrc={therapist.avatarSrc} bgColor={therapist.bgColor} onClick={() => handleTherapistSelect(therapist.id)} />
            </div>)}
        </div>
      </main>
    </div>;
