...
const TherapistSelectionPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className={`flex-grow flex transition-all duration-300 ease-in-out ${showDetails ? 'mr-[500px]' : 'mr-0'}`}>
      ...
      {/* Therapist Details Panel - Fixed Position */}
      {showDetails && selectedTherapist && (
        <div className="fixed right-0 top-0 w-[500px] h-full bg-white shadow-lg overflow-y-auto border-l border-gray-200 transition-all duration-300 ease-in-out z-10">
              ...
              <div className="flex items-start gap-4">
                <div className={`${selectedTherapist.bgColor} w-20 h-20 rounded-full flex items-center justify-center shrink-0`}>
                  <Avatar className="w-16 h-16 border-4 border-white">
                    <AvatarImage src={selectedTherapist.avatarSrc} alt={selectedTherapist.name} className="object-cover" />
                    <AvatarFallback>{selectedTherapist.name[0]}</AvatarFallback>
                  </Avatar>
                </div>
                
                <div>
                  <h2 className="text-2xl font-bold mb-1">{selectedTherapist.name}</h2>
                  <p className="text-base text-gray-600">{selectedTherapist.description}</p>
